...
def export_marked_flagged_reviews(marked_labels, reject_labels):
    ...
    if not marked_ids:
        return "<p style='color:#f85149'>Select at least one row in Mark For Review.</p>", gr.update(visible=False)

    export_rows = []
    conn = get_connection()
    try:
        for row_id in sorted(marked_ids):
            if row_id < 1 or row_id > len(records):
                continue
            record = records[row_id - 1]
            emp_id = str(record.get("emp_id") or "")
            decision = "Reject Payroll" if row_id in reject_ids else "Accept Payroll"
            conn.execute(
                """
                UPDATE flagged_reviews
                SET payroll_decision=?, marked_for_review='Yes'
                WHERE client_code=? AND COALESCE(emp_id,'')=? AND exported_at IS NULL
                """,
                (decision, BATCH.get("client_code"), emp_id),
            )
            export_rows.append(
                {
        ...
        if not export_rows:
            return "<p style='color:#f85149'>Selected rows were not found in the current queue.</p>", gr.update(visible=False)
        conn.execute(
    ...
    if client_user_id and os.path.exists(CLIENT_PORTAL_DB):
        ...
        try:
            ...
            conn.commit()
            send_note = " Sent to client portal."
        finally:
            conn.close()
    log_audit("FLAGGED_REVIEWS_EXPORTED", notes=f"{len(export_rows)} records exported")
    return f"<p style='color:#3fb950'>{len(export_rows)} flagged review record(s) exported.{send_note}</p>", gr.update(value=path, visible=True)
